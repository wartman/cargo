...
var Rabbit = Base.extend({
  ...
  registerMiddleware: function () {
    ...
    return TemplateHandler.register(this)
      .then(MiddlewareHandler.register(this))
      .then(RouteHandler.register(this))
      .then(ErrorHandler.register(this));
  },
  ...
  logStartMessages: function () {
    console.log(
      chalk.yellow.bold('\nRabbit is running!'),
      chalk.grey('\n\nListening on:'),
      chalk.yellow.bold(this.config('server').host + ':' + this.config('server').port),
      chalk.grey('\nCtrl+C to shut down')
    );
  ...
  run: function(externalApp) {
    ...
    return self.registerMiddleware().then(function () {
