...
var Server = Set.extend({
  ...
  addConnection: function (socket) {
    var self = this
    socket._glennId = _.uniqueId('glennId')
    socket.on('close', function () {
      delete self._connections[this._glennId]
    })
    this._connections[socket._glennId] = socket
  },
  ...
  connect: function () {
    ...
    var app = this.getApp()
    var self = this
    // Handle pre-connect stuff here? Or in the main Glenn class?
    ...
    return new Promise(function (resolve) {
      ...
      http.on('error', function (err) {
        ...
        if (err.errno === 'EADDRINUSE') {
          logger.logError(
            '(EADDRINUSE) cannot start Glenn',
            'glenn.Server#connect()',
            'Port ' + self.get('port') + ' is already in use by another program. Is Glenn already running?'
          )
        } else {
          logger.logError(
            '(Code: ' + err.errno + ')',
      ...
      http.on('listening', function () {
        // Ensure Glenn exists correctly on Ctrl-C
  ...
  _logInfo: function (msg, help) {
    Logger.getInstance().logInfo('Glenn Server', msg, help)
  }
