...
// ------
// The primary app
// @todo: remove Set and all the strange 'defineOption' stuff.
//        remove db, auth, etc. (until a later version)
var Rabbit = Set.extend({
  ...
  constructor: function () {
    ...
    this.defineOption('auth', {
      set: false,
      get: function () {
        return this.auth
      }
    })
    this.defineOption('db connection', {
      // Set the correct env
      set: function (value) {
  ...
  _connectServer: function () {
    ...
    // Mount the admin app
    app.use(this.get('admin url'), this.admin.get('app'))
  ...
  _connectDb: function () {
    return this.db
      .set(this.get(['db connection', 'db updates']))
      .connect()
  },
  ...
  _initAuthentication: function () {
    return this.auth
      .set(this.get(['admin url', 'login url', 'db']))
      .init()
  },
  ...
  _initAdmin: function () {
    return this.admin
      .set(this.get(['admin url', 'auth', 'db']))
      .init()
  },
  ...
  run: function (mountPath, app) {
    ...
    return this._connectDb().bind(this)
      .then(this._initAuthentication)
      .then(this._initAdmin)
      .then(this._connectServer)
      .catch(function (err) {
