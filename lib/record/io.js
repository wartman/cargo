...
var IO = Base.extend({
      ...
      return this._readdir(path, options.query)
        ...
        .then(function (files) {
          ...
          return Promise.all(files.map(function (file) {
            ...
            .then(function () {
              return (options.single)? mappedFiles[Object.keys(mappedFiles)[0]] : mappedFiles
            })
        })
    }
  ...
  _readdir: function (pathName, query) {
    var ext = '.' + this.get('extension')
    var _this = this
    return new Promise(function (resolve, reject) {
      fs.readdir(pathName, function (err, files) {
        if (err) {
          ...
          // Then filter with the provided query, if available.
          if (query) files = _this.query(query, files)
          resolve(files)
        }
      })
    })
